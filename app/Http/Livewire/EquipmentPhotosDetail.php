...
class EquipmentPhotosDetail extends Component
{
    use WithPagination;
    use WithFileUploads;
    ...
    {
        $this->editing = false;
        $this->modalTitle = trans('crud.equipment_photos.new_title');
        $this->resetPhotoData();
    ...
    {
        $this->editing = true;
        $this->modalTitle = trans('crud.equipment_photos.edit_title');
        $this->photo = $photo;
    ...
    public function save()
    {
        ...
        if (!$this->photo->equipment_id) {
            $this->authorize('create', Photo::class);

            $this->photo->equipment_id = $this->equipment->id;
        } else {
    ...
    public function toggleFullSelection()
    {
        ...
        foreach ($this->equipment->photos as $photo) {
    ...
    {
        return view('livewire.equipment-photos-detail', [
            'photos' => $this->equipment->photos()->orderBy('id', 'desc')->paginate(5),
        ]);
    }
}
